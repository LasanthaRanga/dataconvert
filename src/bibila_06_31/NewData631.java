...
/**
 * Created by Ranga Rathnayake on 2020-09-02.
 */

public class NewData631 {
    ...
    public static void getData() {
        try {
            ...
            ResultSet data = DB.getData("SELECT all_data_sheet.idAssessment,all_data_sheet.Allocation,all_data_sheet.quarter_value,all_data_sheet.balance0631 FROM all_data_sheet");

            while (data.next()) {
                int idAssessment = data.getInt("idAssessment");
                double quarter_value = data.getDouble("quarter_value");
                double balance0331 = data.getDouble("balance0631");

                System.out.println(data.getInt("idAssessment"));

                if (balance0331 > 0) {
                    System.out.println("++++++++++++++ " + idAssessment);
                    insertQstart(idAssessment, balance0331, quarter_value, 0, quarter_value, 1, 1, 0, 0, 0, 0, 0, 0, 0);

                } else if (balance0331 == 0) {
                    System.out.println("00000000000000 " + idAssessment);

                    insertQstart(idAssessment, 0, quarter_value, 0, quarter_value, 1, 1, 0, 0, 0, 0, 0, 0, 0);
...
//                    } else
                    ...
                    if (quarter_value * 2 <= v) {
                        System.out.println("3333333333333333333");
                        //q2 katama thiyanawa
                        bal = v - quarter_value * 2;
                        insertQstart(idAssessment, 0, 0, quarter_value, quarter_value, 1, 1, 1, 1, 0, 0, quarter_value, quarter_value, bal);
                    } else if (quarter_value <= v) {
                        System.out.println("2222222222222222222");
                        // quate 1 kata thiyanawa
                        bal = v - quarter_value;
                        insertQstart(idAssessment, 0, 0, quarter_value, quarter_value, 1, 1, 0, 0, 0, 0, quarter_value, bal, 0);
                    } else {
                        System.out.println("madi madi madi");
                        //quater 1 kata madi
                        bal = v;
                        insertQstart(idAssessment, 0, 0, quarter_value, quarter_value, 1, 0, 0, 0, 0, 0, bal, 0, 0);
                    }
